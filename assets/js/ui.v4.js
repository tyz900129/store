...
export function mountFooter() {
  ...
  if (f1) f1.addEventListener('submit', async (e) => {
    ...
    if (!email) return;
    const { subscribeEmail } = await import('./data.v4.js');
...
export function updateCartBadge() {
  ...
  if (!badge) return;
  // 通过动态 import 避免循环
  import('./cart.v4.js').then(({ cartCount }) => {
    const n = cartCount();
    badge.textContent = n;
...
export function bindAddToCart(root = document) {
  root.addEventListener('click', async (e) => {
    ...
    const { fetchProduct, FALLBACK } = await import('./data.v4.js');
    const { addToCart } = await import('./cart.v4.js');
